...
**Dart side (v0.6.0+).** `householdProvider` is a keep-alive `Notifier` that seeds its skeleton from `discoveryProvider` and folds `changeEventsProvider` deltas on top through the pure `household_reducer`. It exists because the UI needs an accumulated per-room / per-group model that no single Rust read returns, and because group volume/mute arrive only as events.

Consequences:

- **State survives Flutter *hot reload*, not *hot restart*.** Hot reload keeps the isolate, so both layers persist. A hot restart (or an app relaunch) tears down the Dart isolate: the Rust `StateManager` cache survives in the process, but the Dart accumulator does not, and the fields Dart alone owns - group volume and group mute - are blank until their next NOTIFY.
- A non-Flutter client (e.g. a CLI) could reuse the Rust core unchanged, but would have to re-implement the accumulation `householdProvider` does, and would have no access to group volume/mute except by watching the event stream itself.
