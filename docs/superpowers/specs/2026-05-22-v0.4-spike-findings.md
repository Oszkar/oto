...
**Path A wins** — v0.4 builds on the upstream `sonos-sdk-state` reactive layer (`StateManager` + `SonosEventManager`) rather than raw `sonos-sdk-callback-server` + own change-detection. Cold-start handled by the initial SUBSCRIBE NOTIFY (no separate `.fetch()` step). The non-chosen Path B stays a v0.5 reconsideration point.
...
1. **Correctness — tied.** Both paths: zero warnings, zero errors, every operator action surfaced with sub-second latency, group volume propagated correctly across members, no drops, no out-of-order arrivals, no duplicates.
...
- **Polling on top of GENA.** `sonos-stream::polling::scheduler` polls AVTransport + RenderingControl every 5 s after a 5 s activation delay. This doubles the network traffic to the user's speakers compared to raw-GENA-only. ~0.5 polled events/sec per playing speaker is real LAN traffic — trivial in absolute terms but worth recording as a politeness cost.
