...
| `sonos-api`         | pinned **`=0.8.0`** (exact), aligned with the reactive SDK crates and resolved from crates.io. Direct UPnP SOAP (ZoneGroupTopology / AVTransport / RenderingControl); no `SonosSystem`. SDK 0.8 retired the native-TLS fork (see `LOCAL_PATCHES.md` #2). Don't bump without re-checking SOAP, event delivery, and the hardware acceptance gate. Keep oto's bounded, explicit multicast-egress SSDP until upstream covers both safeguards. |
| FRB surface         | `native/src/api.rs` is the bridge surface (delegates inward, exposes commands and streams events); Rust command functions return `Result`, FRB runs blocking commands off the UI isolate and exposes them to Dart as `Future`s, and events use a `Stream` (v0.4; SDK reactive event path per ARCHITECTURE / sonos-notes). Extending the surface needs an ARCHITECTURE.md update first                                                                                         |
...
**LAN politeness.** The only thing being rate-limited is _us_ against the user's Sonos devices on a home network. `sonos-api` owns SOAP; don't add aggressive polling on top. SSDP discovery is bounded and interface-scoped (§4 / [`tatimblin/sonos-sdk#76`](https://github.com/tatimblin/sonos-sdk/issues/76)).
...
2. **`oto-wire` is the only crate that touches Sonos SDK / network integration crates.** Sole direct `sonos-api` integration point: runs its own multi-interface SSDP and reads topology / playback / state via direct `sonos-api` SOAP (ZoneGroupTopology / AVTransport / RenderingControl). Live events terminate in `oto-wire` via the SDK reactive state/event layer - **never** `SonosSystem` (the umbrella was dropped at v0.3). Upstream fixed per-interface probing in 0.5.3 ([#76](https://github.com/tatimblin/sonos-sdk/issues/76)), but 0.8 still lacks explicit multicast egress selection and an absolute receive deadline. Those safeguards keep SSDP in `oto-wire`.
...
5. **`oto_native` is glue only.** No business logic in `native/src/api.rs`; it delegates inward. Rust command functions return `Result`, Dart receives `Future`s, and events use a `Stream`.
...
Every task: (1) identify the crate/layer; (2) list invariants affected (§4 boundary, §2.1 convention); (3) smallest safe diff at the root cause; (4) validate (§6); (5) ambiguous → ask early. Avoid drive-by refactors. Use the context7 or dart-mcp-server MCP to verify FRB / sonos-api / Riverpod APIs rather than guessing.
...
- DO NOT use the em-dash Unicode character (U+2014), use regular hyphens (-) instead including in code, PR descriptions, everywhere.
