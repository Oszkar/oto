...
| v0.5 | **Hardening before UI** — topology change events, group form/break, Android `MulticastLock`, model repopulate, in-band subscription-failure surfacing. |
...
Released: `v0.1.0`, `v0.2.0`, and `v0.3.0`. Commands are non-sync Dart `Future`s (every command is a blocking SOAP round-trip); live property changes flow as a Dart `Stream` (v0.4, feature-complete on `main`; release tag pending hardware-acceptance gate). Milestone status, forward plan, and known caveats: [docs/ROADMAP.md](docs/ROADMAP.md). System design: [docs/ARCHITECTURE.md](docs/ARCHITECTURE.md). Per-release detail: [CHANGELOG.md](CHANGELOG.md).
